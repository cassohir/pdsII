...
class String2Int : public Mapeador<std::string, int>
{
  ...
  {
    int transformado = -1;
    std::istringstream(d) >> transformado;
    if (transformado != -1)
      return transformado;
    else
      return -1;
...
class FiltroNumTriangular : public Filtro<int>
{
  ...
  {
    int inesimo = 0;
    while (inesimo * (inesimo + 1) / 2 < num + 1)
    {
      if (inesimo * (inesimo + 1) / 2 == num)
        return true;
      else
        ++inesimo;
    }
...
void convert2int(std::vector<std::string> &in, std::vector<int> &out)
{
  for (auto i : in)
  {
    ...
    if (filtro->dadoValido(i))
      out.push_back(mapeador->transformaDado(i));
    delete filtro;
    delete mapeador;
  }
}
